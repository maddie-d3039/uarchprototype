...
void icache_access(int addr,
                   int data_way0[cache_line_size],
                   int data_way1[cache_line_size],
                   I$_TagStoreEntry *tag_metadata)
{
    int bank = (addr >> 3) & 0x1;
    int set = (addr >> 4) & 0x7;

    memcpy(data_way0,
           icache.data.icache_datastore[bank][set][0],
           sizeof(int) * cache_line_size);

    memcpy(data_way1,
           icache.data.icache_datastore[bank][set][1],
           sizeof(int) * cache_line_size);

    *tag_metadata = icache.tag.icache_tagstore[bank][set];
}

void dcache_access(int addr,
                   int **data_way0,
                   int **data_way1,
                   int **data_way2,
                   int **data_way3,
                   D$_TagStoreEntry *tag_metadata)
{
    int bank = (addr >> 3) & 0x1; // two banks
    int set = (addr >> 4) & 0x3;  // four sets (0..3)

    // each [bank][set] entry has 4 “ways” of cache_line_size ints
...
void addgen_branch_stage()
{
    if (new_pipeline.agbr_valid && need_prediction)
