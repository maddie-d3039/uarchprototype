            ...
            if (thisRS.entries[j].old_bits == search_age)
            {
                if (thisRS.entries[j].op1_ready == FALSE || thisRS.entries[j].op2_ready == FALSE)
                {
                    ...
                    else if (thisRS.entries[j].op1_addr_mode == indexed)
                    {
                        ...
                        {
                            thisRS.entries[j].op1_ready = TRUE;
                            thisRS.entries[j].op1_combined_val = thisRS.entries[j].op1_index_val;
                            evaluated_op1 = true;
                        }
                    }
                    else if (thisRS.entries[j].op1_addr_mode == based_indexed)
                    {
                        ...
                        {
                            thisRS.entries[j].op1_ready = TRUE;
                            thisRS.entries[j].op1_combined_val = thisRS.entries[j].op1_base_val + thisRS.entries[j].op1_index_val;
                            evaluated_op1 = true;
                        }
                    }
                    else if (thisRS.entries[j].op1_addr_mode == based_indexed_disp)
                    {
                        ...
                        {
                            thisRS.entries[j].op1_ready = TRUE;
                            thisRS.entries[j].op1_combined_val = thisRS.entries[j].op1_base_val + thisRS.entries[j].op1_index_val + thisRS.entries[j].op1_imm;
                            evaluated_op1 = true;
                        }
                    }
                    else if (thisRS.entries[j].op1_addr_mode == scaled_index)
                    {
                        if (thisRS.entries[j].op1_base_valid == TRUE && thisRS.entries[j].op1_index_valid == TRUE && thisRS.entries[j].op1_ready == FALSE)
                        {
                            thisRS.entries[j].op1_ready = TRUE;
                    ...
                    else if (thisRS.entries[j].op1_addr_mode == based_scaled_disp)
                    {
                        if (thisRS.entries[j].op1_base_valid == TRUE && thisRS.entries[j].op1_index_valid == TRUE && thisRS.entries[j].op1_ready == FALSE)
                        {
                            thisRS.entries[j].op1_ready = TRUE;
                    ...
                    else if (thisRS.entries[j].op2_addr_mode == indexed)
                    {
                        ...
                        {
                            thisRS.entries[j].op2_ready = TRUE;
                            thisRS.entries[j].op2_combined_val = thisRS.entries[j].op2_index_val;
                            evaluated_op2 = true;
                        }
                    }
                    else if (thisRS.entries[j].op2_addr_mode == based_indexed)
                    {
                        ...
                        {
                            thisRS.entries[j].op2_ready = TRUE;
                            thisRS.entries[j].op2_combined_val = thisRS.entries[j].op2_base_val + thisRS.entries[j].op2_index_val;
                            evaluated_op2 = true;
                        }
                    }
                    else if (thisRS.entries[j].op2_addr_mode == based_indexed_disp)
                    {
                        ...
                        {
                            thisRS.entries[j].op2_ready = TRUE;
                            thisRS.entries[j].op2_combined_val = thisRS.entries[j].op2_base_val + thisRS.entries[j].op2_index_val + thisRS.entries[j].op2_imm;
                            evaluated_op2 = true;
                        }
                    }
                    else if (thisRS.entries[j].op2_addr_mode == scaled_index)
                    {
                        if (thisRS.entries[j].op2_base_valid == TRUE && thisRS.entries[j].op2_index_valid == TRUE && thisRS.entries[j].op2_ready == FALSE)
                        {
                            thisRS.entries[j].op2_ready = TRUE;
                    ...
                    else if (thisRS.entries[j].op2_addr_mode == based_scaled_disp)
                    {
                        if (thisRS.entries[j].op2_base_valid == TRUE && thisRS.entries[j].op2_index_valid == TRUE && thisRS.entries[j].op2_ready == FALSE)
                        {
                            thisRS.entries[j].op2_ready = TRUE;
    ...
    else if (pipeline.rr_op1_addr_mode == direct)
    {
        ...
        for (int i = 0; i < max_sq_size; i++)
        {
            if (sq.entries[i].entry_valid == FALSE)
            {
             ...
             pipeline.rr_op1_addr_mode == scaled_index ||
             pipeline.rr_op1_addr_mode == based_scaled_disp)
    {
        thisRS.entries[entry_index].old_bits = thisRS.occupancy;
        thisRS.occupancy++;
        thisRS.entries[entry_index].entry_valid = TRUE;
    ...
    else if (pipeline.rr_op2_addr_mode == direct)
    {
        ...
        if (found_data == false && found_tag == false)
        {
            ...
            thisRS.entries[entry_index].op2_load_alias_valid = TRUE;
             ...
             pipeline.rr_op2_addr_mode == scaled_index ||
             pipeline.rr_op2_addr_mode == based_scaled_disp)
    {
        thisRS.entries[entry_index].old_bits = thisRS.occupancy;
        thisRS.occupancy++;
        thisRS.entries[entry_index].entry_valid = TRUE;
...
void addgen_branch_stage()
{
    if (new_pipeline.agbr_valid && pipeline.agbr_cs[control_instruction_type])
