...
class FAT
{
    ...
    FlagAliasPool *aliases = new FlagAliasPool();
    ...
    bool makeAliases(int *cs_line)
    { // make a class for the control store
        int alias = getAlias();
        if (alias == -1)
        {
            return false;
    ...
    void writeback(int executeVal, int alias)
    { // add more params later, not sure what signals needed to set different flags
        if (executeVal < 0)
        {
            if (tag[SF] == alias)
            {
